...
import ModalCreatePlaces from '../../modal/CreatePlaces';
...
const ListPlaces = () => {
    ...
    const [rowList, setRowList] = useState<Places[]>([]);
    ...
    useEffect(()=>{
        ...
        setTimeout(()=>{
          ...
          // dispatch icon title
          storeMenu.dispatch({type:'titleicon', text: 'Places'})

          // dispatch breadcrumb path
          storeMenu.dispatch({type:'breadcrumbs', text:[{key:'Maintenance', value:'Maintenance'}, {key:'Places', value:'Places'}]})
    ...
    let columns = useMemo<MRT_ColumnDef<Places>[]>(
      ()=> 
        [
          {
            accessorKey: 'name',
    ...
    const getDataList = () => {
      ...
      setShowSkeleton(true);
        ...
        PPE_getApiSync(`${URL_API_PPE}/v1/places/?skip=0&limit=1000`
              ,null
              , 'application/json'
              , 'GET'
    ...
    const toolbarButtonClick = async(status:'Create'|'Edit'|'Delete', title, row?) => {
      ...
      if (status == 'Delete'){
          handleSwal('Delete', row.original?.['name'], (resultSwal:SweetAlertResult)=>{
              if (resultSwal.isConfirmed){
                  handleSwalResult('isConfirmed', row.original?.['name']
                      ,{
                          endpoint:`${URL_API_PPE}/v1/places/${row.original?.['id']}`, obj:null, body_type:'application/json', method:'DELETE', token:getValueFromLocalStorageFunc("IOT_TOKEN")
                        }
                      , (res)=>{
                    ...
                    renderTopToolbarCustomActions={({table})=>{
                        ...
                        return (
                          <Box>
                              <div className='d-flex gap-1'>
                                  {
                                      getValueFromLocalStorageFunc("IOT_IS_SPUSR") == "true" && (
                                          <Button color="info" onClick={()=>{toolbarButtonClick('Create','Create Place')}} variant='contained'
                              ...
                              <ModalCreatePlaces par_show={showModal ?? false} title={modalTitle} status={modalStatus} 
                    ...
                    renderRowActions={({cell, row, table}) => {
                        ...
                        return (
                          <div className='d-flex align-items-center gap-2'>
                              <div className={`${styles['ppe-icon-actions']} ${styles['edit']}`} title="Edit" onClick={()=>{toolbarButtonClick('Edit','Edit Place', row)}}>
                                  <ModeEdit sx={{fill: 'mediumslateblue !important'}} />
                              </div>

                              <div className={`${styles['ppe-icon-actions']} ${styles['delete']}`} title="Hapus" onClick={()=>{toolbarButtonClick('Delete','Delete Place', row)}}>
