...
import { AppRegistration, Architecture, Check, LensTwoTone } from '@mui/icons-material';
...
const ModalCreateResults = (props) => {

  const { par_show, title, status, row, tipeMenu, par_statusLoader, listFeatures, listCameras, outChange } = props;
  ...
  // listFeatures -> data master feature (select)
  // listCameras -> data master camera (select)
  ...
  const objRequired = ['Camera', 'Feature', 'Raw Data']
  ...
  const [optionsCamera, setOptionsCamera] = useState<any>([]);
  const [valueFeatureSelect, setValueFeatureSelect] = useState<{}|null>({});
  ...
  useEffect(()=>{
    setOptionsFeature([...listFeatures]);
    setOptionsCamera([...listCameras]);

  },[listFeatures, listCameras])
        ...
        // setTimeout(()=>{
            ...
            if (typeof statusCodeError != 'undefined'){
                ...
                if (Array.isArray(response)){
                  let dataTemp = response.map((objres)=>{
                        return {
                          'value': objres?.['id'],
                          'label': objres?.['feature_name']
                        }
                  });

                  setOptionsFeature([...dataTemp]);
                }
            }
        // },500)
    });
  ...
  useEffect(()=>{
      ...
      if (status == 'Edit'){
          ...
          if (Array.isArray(optionsFeature)){
            let findItem = optionsFeature.find(x=>x?.['value'] == row?.['id_feature']);
            if (findItem){
              // set value ke <Select
              setValueFeatureSelect({...findItem});
            }
          }

          if (Array.isArray(optionsCamera)){
            let findItem = optionsCamera.find(x=>x?.['value'] == row?.['id_camera']);
            if (findItem){
              // set value ke <Select
          ...
          // setValueFeatureSelect
        
          setObjValueInput((obj)=>{
              return {
                'Camera': row?.['id_camera'] ?? null,
                'Feature': row?.['id_feature'] ?? null,
  ...
  const onChangeSelectFilterModal = (e, action, menu:'feature'|'camera') => {
      if (action != 'clear')
      {
        switch (menu) {
          case 'feature':
            setValueFeatureSelect({value:e.value, label:e.label});break;
          case 'camera':
            setValueCameraSelect({value:e.value, label:e.label});break;
        }
        ...
        setObjValueInput((obj:any)=>{
          return {
            ...obj,
            [menu=='feature'?'Feature':'Camera']:e.value
          }
        })
      }
      else {
        switch (menu) {
          case 'feature': setValueFeatureSelect(null);break;
          case 'camera': setValueCameraSelect(null);break;
        }

        setObjValueInput((obj:any)=>{
          return {
            ...obj,
            [menu == 'feature'?'Feature':'Camera']:null
          }
        })
      }
      // setValueFeatureSelect()
  ...
  const handleSubmit = () => {
    ...
    // {
        if (status == 'Create'){
          methodhit = 'POST';
          endpoint = `${URL_API_PPE}/v1/results`;
          ...
          methodhit = 'PUT';
          endpoint = `${URL_API_PPE}/v1/results/${row?.['id']}`;
        ...
        objSubmitToAPI = 
          {
            ...
            "id_feature": objValueInput?.['Feature'],
            "raw_data": objValueInput?.['Raw Data'],
          }
    // }
            ...
            <Modal.Header className={`modalheader modal-header-global ${status == 'Edit' ? 'edit':'create'}`}>
                <Modal.Title className='dashboard-modal-title' >
                    
                    {
                      status == 'Create' && (
                        <>
                            <AppRegistration style={{position:'absolute', right:15, top:25, zIndex:0, transform:`scale(1.5)`}} 
...

                    {
                      status == 'Edit' && (
                        <>
                            <AppRegistration style={{position:'absolute', right:20, top:25, zIndex:0, transform:`scale(1.5)`}} 
                                          className='login-dropdown-icon'/>
                            {/* <FontAwesomeIcon icon = {faCamera} className='login-dropdown-icon' color={'darkgreen'} 
                                    style = {{position:'absolute', right:15, top:25, zIndex:0, transform:`scale(2)`}}/> */}
...

                {
                    ...
                    <div
                        ...
                        <Form.Group className='mb-1 col-12' controlId='id-camera'>
                            <Form.Label className='mb-1 dash-modal-form-label required'>Camera</Form.Label>
                            <Select options={optionsCamera} 
                                value={valueCameraSelect}
                                tabSelectsValue={false} isClearable={true}  
                                isSearchable={true}
                                onChange={(e, {action})=>{onChangeSelectFilterModal(e, action, 'camera')}}
                            />
                        </Form.Group>

                        <Form.Group className='mb-1 col-12' controlId='id-feature'>
                            <Form.Label className='mb-1 dash-modal-form-label required'>Feature</Form.Label>
                            <Select options={optionsFeature} 
                                ...
                                value={valueFeatureSelect}
                                tabSelectsValue={false} isClearable={true}  
                                isSearchable={true}
                                onChange={(e, {action})=>{onChangeSelectFilterModal(e, action, 'feature')}}
                            />
                            {/* <Form.Control type = "text" className='modal-input-placeholder'
                                placeholder="Enter Feature Name"
                                maxLength={100}
                                ref={refEmailInput}
                                value={objValueInput?.['Feature Name']}
                                disabled={loaderStatus}
                                onChange={(event)=>handleChangeInput(event, 'Feature Name')}
                            /> */}
                        </Form.Group>

                        <Form.Group className='mb-1 col-12' controlId='id-rawdata'>
                            <Form.Label className='mb-1 dash-modal-form-label required'>Raw Data</Form.Label>
                            <Form.Control type = "text" className='modal-input-placeholder'
                                placeholder="Enter Raw Data"
                                maxLength={100}
                                ref={refFullNameInput}
                                value={objValueInput?.['Raw Data']}
                                disabled={loaderStatus}
                                onChange={(event)=>handleChangeInput(event, 'Raw Data')}
                            />
                        </Form.Group>
                    </div>
                }
...
export default ModalCreateResults