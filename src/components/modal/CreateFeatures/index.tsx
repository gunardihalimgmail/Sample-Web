...
import { Architecture, Check, LensTwoTone } from '@mui/icons-material';
...
const ModalCreateFeatures = (props) => {
  ...
  const objRequired = ['Feature Name', 'Service Name']
  ...
  const handleSubmit = () => {
    ...
    // {
        if (status == 'Create'){
          methodhit = 'POST';
          endpoint = `${URL_API_PPE}/v1/features`;
          ...
          methodhit = 'PUT';
          endpoint = `${URL_API_PPE}/v1/features/${row?.['id']}`;
      ...
      // object create / edit dengan kondisi sama
        objSubmitToAPI = 
          {
            ...objSubmitToAPI,
            "feature_name": objValueInput?.['Feature Name'],
            "service_name": objValueInput?.['Service Name']
          }
    // }
            ...
            <Modal.Header className={`modalheader modal-header-global ${status == 'Edit' ? 'edit':'create'}`}>
                <Modal.Title className='dashboard-modal-title' >
                    
                    {
                      status == 'Create' && (
                        <>
                            <Architecture style={{position:'absolute', right:15, top:25, zIndex:0, transform:`scale(2)`}} 
...

                    {
                      status == 'Edit' && (
                        <>
                            <Architecture style={{position:'absolute', right:20, top:25, zIndex:0, transform:`scale(2)`}} 
                                          className='login-dropdown-icon'/>
                            {/* <FontAwesomeIcon icon = {faCamera} className='login-dropdown-icon' color={'darkgreen'} 
                                    style = {{position:'absolute', right:15, top:25, zIndex:0, transform:`scale(2)`}}/> */}
...

                {
                    ...
                    <div
                        ...
                        <Form.Group className='mb-1 col-12' controlId='id-name'>
                            <Form.Label className='mb-1 dash-modal-form-label required'>Feature Name</Form.Label>
                        ...
                        <Form.Group className='mb-1 col-12' controlId='id-cameralocation'>
                            <Form.Label className='mb-1 dash-modal-form-label required'>Service Name</Form.Label>
                            <Form.Control type = "text" className='modal-input-placeholder'
                                placeholder="Enter Service Name"
                                maxLength={100}
                                ref={refFullNameInput}
                                value={objValueInput?.['Service Name']}
                                disabled={loaderStatus}
                                onChange={(event)=>handleChangeInput(event, 'Service Name')}
                            />
                        </Form.Group>
                    </div>
                }
...
export default ModalCreateFeatures