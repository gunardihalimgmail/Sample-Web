...
const DashboardAnalitik = () => {
  ...
  useEffect(()=>{
    ...
    const fetchData = async() => {
        ...
        setTimeout(()=>{
          // dispatch icon title
          storeMenu.dispatch({type:'titleicon', text: 'PPE'});
  
          // dispatch breadcrumb path
          storeMenu.dispatch({type:'breadcrumbs', text:[{key:'Dashboard', value:'Dashboard'}, {key:'Analitik', value:'Analitik'}, {key:'PPE', value:'PPE'}]});
  ...
  const generateCameraAndTools = async(arr_camera?:any[], index_camera?:any, tanggal?:Date|null) => {
    ...
    if (index_camera != null && !isNaN(index_camera)) // jika angka, maka masuk
    {
        ...
        if (dataCamera_single != null)
        {
            ...
            let resultdata = await PPE_getApiSync(`${URL_API_PPE}/v1/dashboard/ppe-statistics?skip=0&limit=1000&id_camera=${camera_id}&start_month=${start_month}&end_month=${end_month}`
                  , null
                  , 'application/json'
                  , 'GET'
                  , getValueFromLocalStorageFunc("IOT_TOKEN") ?? '');
