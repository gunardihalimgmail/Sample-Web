    ...
    // arrImageCamera -> {isLoading: true, url: `https://picsum.photos/2000/${Math.floor((Math.random()*401)+400)}`}
      ...
      const generateArrImageCameraFunc_InComp = async(id_camera?:any, tanggal?:Date|null) => {
          ...
          if (id_camera != null && !isNaN(id_camera)) // jika angka, maka masuk
          {
                  ...
                  let resultdata = await PPE_getApiSync(`${URL_API_PPE}/v1/dashboard/vehicle-image?skip=${arrImageCameraGal.length}&limit=100&id_camera=${camera_id}&start_month=${start_month}&end_month=${end_month}`
                        , null
                        , 'application/json'
                        , 'GET'
            ...
            ? 
              (
                  <>
                                                ...
                                                <Card.Body>
                                                    ...
                                                    <div className='ppe-card-body-title mt-3'>
                                                        <div className='d-flex justify-content-center'>
                                                          {/* <span>{objCameraAndTools?.['tools']?.[tool]?.['value']}</span> */}
                                ...
                                <div className='mt-3 ppe-card-dash-info-image-gal'>
                                    ...
                                    <div className='d-flex align-items-start ppe-card-dash-info-image-title'
                                        ...
                                        {/* <div className='p-3' style = {{color:'blue', 
                                                height:`${heightGalContainer}px`, border:'1px solid red'}}>
                                            GOOD
                                        </div> */}
                      ...
                      <div className='mt-5' style={{cursor:'auto', width:'100%'}}>
...
const DashboardVehicle = () => {
  ...
  const generateArrImageCameraFunc = async(arr_camera?:any[], index_camera?:any, tanggal?:Date|null) => {
    ...
    if (index_camera != null && !isNaN(index_camera)) // jika angka, maka masuk
    {
        ...
        if (dataCamera_single != null)
        {
            ...
            let resultdata = await PPE_getApiSync(`${URL_API_PPE}/v1/dashboard/vehicle-image?skip=0&limit=100&id_camera=${camera_id}&start_month=${start_month}&end_month=${end_month}`
                  , null
                  , 'application/json'
                  , 'GET'
                  , getValueFromLocalStorageFunc("IOT_TOKEN") ?? '');
  ...
  useEffect(()=>{
    ...
    const fetchData = async() => {
        ...
        setTimeout(()=>{
          // dispatch icon title
          storeMenu.dispatch({type:'titleicon', text: 'Vehicle'});
  
          // dispatch breadcrumb path
          storeMenu.dispatch({type:'breadcrumbs', text:[{key:'Dashboard', value:'Dashboard'}, {key:'Analitik', value:'Analitik'}, {key:'Vehicle', value:'Vehicle'}]});
...

            {
               dataCamera != null && (Array.isArray(dataCamera) && dataCamera.length > 0) && (
                    <>
                        ...
                        >
                            {
                              // Object.keys(objCardList).map((cam, idx)=>{
                              dataCamera?.map((obj_cam, idx)=>{
                                return (
                                  // <Tab disabled={disabledTab} key={`tab-ply-${idx}`} label={`${objCardList?.[cam]?.['label']}`} />
                                  <Tab disabled={disabledTab || !obj_cam?.['is_active']} key={`tab-ply-${idx}`} label={`${obj_cam?.['camera_name']}`} />
                                )
                              })
                            }
                        </Tabs>
                        ...
                        <div className='mt-4 px-3'>
                          ...
                          
                            {
                              !showLoaderMain && (
                                <>
                                  {/* // looping camera -> tools (di dalam CustomTabPanel) */}
                                  <div>
                                    {
                                      dataCamera != null && (Array.isArray(dataCamera) && dataCamera.length > 0) && (
                                      // Object.keys(objCardList).map((cam, idx)=>{
                                          ...
                                          // return (
                                              // <NumberAnimate key={`tab-custom-${idx}`} propStyle={{textColor:'black'}} angka={1230} />
                                          <>
                                              {
                                                dataCamera?.map((obj_cam, idx)=>{
                                                  return (
                                                    ...
                                                    //     <div key={`${'cam-' + idx}`} style={{height:'70px'}}>{JSON.stringify(obj_cam)}</div>

                                                    //     <div key={`${'objcardlist-' + idx}`}>{JSON.stringify(objChartData?.[obj_cam?.['id']])}</div>

                                                    //     <div style={{height:'30px'}}></div>
                                                    // </div>

                                                      <CustomTabPanel key={`tab-custom-${idx}`} index={idx} value={tabValue} 
                                                              objCameraAndTools={objCardList?.[obj_cam?.['id']]}  // element tools objCardList
                                                              objChartData={objChartData?.[obj_cam?.['id']]}  // configurasi bar chart
                                                      ...
                                                      // <CustomTabPanel key={`tab-custom-${idx}`} index={idx} value={tabValue} 
                                                      //         objCameraAndTools={objCardList?.[cam]}  // element tools objCardList
                                                      //         objChartData={objChartData?.[cam]}  // configurasi bar chart
                                                      //         arrImageCamera={arrImageCamera}
                                                      //         idCamera={cam}
                                                      //         tanggal={startDate}
                                                      //         outRefresh={outRefreshTabPanel}
