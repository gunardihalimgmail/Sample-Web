...
const Recordings = () => {
  ...
  const getDataRecordings_Scroll = async(place_id) => {
    ...
    if (place_id != null) {
      ...
      let resultdata = await PPE_getApiSync(`${URL_API_PPE}/v1/logs/places/?place_id=${place_id}&skip=${datas.length}&limit=7&start_month=${start_date}&end_month=${end_date}`
                , null
                , 'application/json'
                , 'GET'
  ...
  const generateVideo_Scroll = async(tanggal_start?:Date, tanggal_end?:Date, place_id?:any) => {
    ...
    { 
          ...
          if (place_id != null) {
              let result:any = await getDataRecordings_Scroll(place_id);
              
              if (Array.isArray(result)) {

                setInputRefs(prevRefs=>{
                      const newRefs:any = Array.from({length:((result.length ?? 0) + (datas.length ?? 0))},(_,i)=> prevRefs[i] || React.createRef());
        
                      return newRefs;
                });

                // set ke Playing = false semua video (posisi pause)
                setPlayingVideo(prev=>{
                    const newPlaying:any = Array.from({length:((result.length ?? 0) + (datas.length ?? 0))},(_,i)=> false);
                    return newPlaying;
                });
    ...
    if (scrollTop >= threshold) {
      ...
      setTimeout(async()=>{
        ...
        if (place_id != null) {
            ...
            await generateVideo_Scroll(startDate, endDate, place_id);
  ...
  // AWS Credentials
  const accessKeyId = '00ca8cb649616187797f';
  const secretAccessKey = 'ks62I4livyQnVtkwb3a8pEi0Iq98m0D/MQD5Os5P';
  const region = 'us-east-1';
  const service = 's3';
  const url = 'https://s3-id-jkt-1.kilatstorage.id/analytic/record/video.mp4';
  ...
  // useEffect(()=>{
      // AWS Credentials for video mp4

      // const url = 'https://s3-id-jkt-1.kilatstorage.id/analytic/record/stream_20240813_191427.mp4';
      ...
      //   const signingKey = getSignatureKey(secretAccessKey, dateStamp, region, serviceName);
  ...
  // },[accessKeyId, secretAccessKey, region, service, url])
  ...
  useEffect(()=>{
    ...
    const fetchData = async() => {
        ...
        setTimeout(()=>{
          // dispatch icon title
          storeMenu.dispatch({type:'titleicon', text: 'Recordings'});
  
          // dispatch breadcrumb path
          storeMenu.dispatch({type:'breadcrumbs', text:[{key:'Dashboard', value:'Dashboard'}, {key:'Monitoring', value:'Monitoring'}, {key:'Recordings', value:'Recordings'}]});
    ...
    return ()=>{
      ...
      // clean react player
        if (Array.isArray(inputRefs) && inputRefs.length > 0) {
          setInputRefs(prevRefs=>{
              let temp_ref = [...prevRefs];
              
              inputRefs.forEach((refChild, idx)=>{
                    if (refChild.current){
                      const player = refChild.current.getInternalPlayer();

                      inputRefs[idx].current.seekTo(0);
                    }
              });
              return prevRefs
          });
  ...
  const getDataRecordings = async(place_id) => {
    ...
    if (place_id != null) {
      ...
      let resultdata = await PPE_getApiSync(`${URL_API_PPE}/v1/logs/places/?place_id=${place_id}&skip=0&limit=7&start_month=${start_date}&end_month=${end_date}`
                , null
                , 'application/json'
                , 'GET'
                , getValueFromLocalStorageFunc("IOT_TOKEN") ?? '');
  ...
  const handleCari = async() => {
      ...
      if (selectedSite.length > 0 && selectedPlace.length > 0
          ...
          let place_id = selectedPlace?.[0]?.id;
          if (place_id != null) {
              let result:any = await getDataRecordings(place_id);

              if (Array.isArray(result)) {

                if (result.length > 0){
                  setInputRefs(prevRefs=>{
                        const newRefs:any = Array.from({length:result.length},(_,i)=> prevRefs[i] || React.createRef());
                  ...
                  // set ke Playing = false semua video (posisi pause)
                  setPlayingVideo(prev=>{
                      const newPlaying:any = Array.from({length:result.length},(_,i)=> false);
                      return newPlaying;
                  });
                }
  ...
  const handleTesVid = () => {
    setPlayingVideo((prevRef)=>{
      let temp:any = [...prevRef];
      let final:any = [...temp.map((val, idx)=>false)];
      return [...final];
    });
    ...
    <>
        <div className={styles['ppe-anly']}>
            ...
            <div className={`d-flex flex-column flex-sm-column flex-md-row align-items-stretch gap-2 ${styles['ppe-anly-datepicker']}`}>
                ...
                <div className={`d-flex align-items-center justify-content-center ${styles['recordings-strip-between-date']}`}>
                    <span>-</span>
                </div>
                                  ...
                                  <div className='d-flex flex-column flex-sm-column flex-md-row flex-wrap gap-3'>
                                      {
                                        ...
                                        datas.map((obj, idx)=>(
                                            ...
                                            <div key={`live-video-${idx}`} className={`${styles['live-col-video']}
                                                      ${styles[`recordings-col-video-color`]}
                                                  `}>
    
                                                <div className='d-flex justify-content-start flex-column' style={{overflow:'hidden'}}>
                                                    ...
                                                    <div className='border' style={{height:'auto'}}>
                                                      ...
                                                      {/* // src={`https://cctvjss.jogjakota.go.id/margo-utomo/Toko-Cat-Wahyu.stream/chunklist_w1093883685.m3u8`} */}
                                                        ...
                                                        {/* *.mp4 pakai ReactPlayer*/}
                                                        {/* setting mp4 local di declaration.d.ts, lalu inject di "include" tsconfig.json */}
    
                                                          <ReactPlayer 
                                                                // url="/assets/video/video1.mp4"   // dari public/assets
                                                                // url={`${idx==0 ? video1 : video2}`}
                                                                ref={inputRefs?.[idx] ?? null}
                                                                url={`${obj?.['url_video'] || null}`}
                                                                // url={videoUrl}
                                                                muted={true}  
                                                                controls={true}
                                                                loop={false}
                                                          ...
                                                          {/* <video controls crossOrigin="anonymous" width={`100%`}>
                                                              <source src={`${obj?.['url_video']}`} type="video/mp4" />
                                                    ...
                                                    <div className={`d-flex align-items-center justify-content-between ${styles['recordings-bg']} py-2`}>
    
                                                        {/* Periode Tanggal */}
                                                        <h6 className={`ms-2 ${styles['recordings-label-title-1']}`}>{obj?.['tanggal'] || ''}</h6>
                                                        
                                                        <h6 className={`me-2 ${styles['recordings-label-title-2']}`}>{obj?.['waktu'] || ''}</h6>
    
                                                    </div>
    
                                                    {/* *** Label Nama Camera */}
                                                    <div className={`d-flex align-items-center justify-content-between ${styles['recordings-bg']} pb-3 pt-2 ps-2`}>
    
                                                        {/* Periode Tanggal */}
                                                        <div className={`d-flex justify-content-start align-items-center`}>
                                                            <span className={`${styles['recordings-camera-icon']}`}></span>
                                                            <span className={`${styles['recordings-camera-name']} ms-2`}>{obj?.['camera_name']}</span>
                                                        </div>
